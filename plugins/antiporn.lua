...
os.rename(result, file)
  local curl = 'curl -X POST "https://api.sightengine.com/1.0/nudity.json" -F "api_user=95982217" -F "api_secret=kTdmp86nn4LbsUM8 	
" -F "photo=@'..file..'"'
...
if jdat.status then
      if jdat.status == 'failure' then
         ...
         send_large_msg(get_receiver(msg), 'این یک تصویر معمولی است', ok_cb, false)
         end
  end
end
  else
